...
/**
 *  Creamos un customHook para obtener la descripcion de un producto,
...
export const useProductDescription = (productId: string) => {
  const query = useQuery({
    queryKey: ["productDescription", productId], // con esto le estamos diciendo a React Query que almacene nuestra peticion en cache mediante un identificador unico como lo es el id del producto, y la Key identificadora de nuestra petición.
    queryFn: () => getProductDescription(productId), // le pasamos nuestra función que realiza la petición, para que internamente React Query la almacene en cache y cree los estados correspondientes.
  });
