...
export const useSearchProduct = (productTerm: string) => {
  const query = useQuery({
    queryKey: ["products", productTerm], // con esto le estamos diciendo a React Query que almacene nuestra peticion en cache mediante un identificador unico como lo es el termino de busqueda del producto, y la Key identificadora de nuestra petición.
    queryFn: () => searchProduct(productTerm), // le pasamos nuestra función que realiza la petición, para que internamente React Query la almacene en cache y cree los estados
  });
